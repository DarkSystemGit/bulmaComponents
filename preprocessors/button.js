...
module.exports=(comp,obj)=>{
    ...
    obj.classes.forEach(className=>{
        document.getElementsByClassName('button')[0].className = `${elm.getAttribute("class")} ${className}`
    })
    Object.keys(obj.attributes).forEach(attr=>{
        document.getElementsByClassName('button')[0].setAttribute(attr,obj.attributes[attr])
    })
